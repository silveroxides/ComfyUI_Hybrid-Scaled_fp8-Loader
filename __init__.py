...
class ScaledFP8HybridUNetLoader:
    ...
    def load_unet(self, model_name, model_type, force_fp8_matmul):
        unet_path = folder_paths.get_full_path("unet", model_name)
        # Configure ops with metadata inspection only (no tensor loading)
        hybrid_fp8_ops.configure_hybrid_ops(model_path=unet_path, model_type=model_type, force_fp8_matmul=force_fp8_matmul)
...
class ScaledFP8HybridCheckpointLoader:
    ...
    def load_checkpoint(self, ckpt_name, model_type, force_fp8_matmul):
        ckpt_path = folder_paths.get_full_path("checkpoints", ckpt_name)
        # Configure ops with metadata inspection only (no tensor loading)
        hybrid_fp8_ops.configure_hybrid_ops(model_path=ckpt_path, model_type=model_type, force_fp8_matmul=force_fp8_matmul)
