...
def setup_hybrid_ops(model_path, keep_distillation, keep_nerf):
...
class ScaledFP8HybridUNetLoader:
    ...
    def load_unet(self, model_name, keep_distillation, keep_nerf):
        unet_path = folder_paths.get_full_path("unet", model_name)
        ops = setup_hybrid_ops(unet_path, keep_distillation, keep_nerf)
...
class ScaledFP8HybridCheckpointLoader:
    ...
    def load_checkpoint(self, ckpt_name, keep_distillation, keep_nerf):
        ckpt_path = folder_paths.get_full_path("checkpoints", ckpt_name)
        ops = setup_hybrid_ops(ckpt_path, keep_distillation, keep_nerf)
