...
def configure_hybrid_ops(model_path, model_type="none", force_fp8_matmul=False, scale_input_enabled=None, disable_fp8_mat_mult=False):
    """
    ...
    if model_path is not None:
        ...
        try:
            with utils.MemoryEfficientSafeOpen(model_path, device="cpu") as f:
                # Use header directly - no need to call keys() which creates a list
                header = f.header
                
                # Get dtype from metadata without loading tensors
                for key, metadata in header.items():
                    # Skip metadata and non-weight keys
                    ...
                    for substring in high_precision_substrings:
                        if substring in key:
                            ...
                            excluded_layers_dtype[key] = utils.MemoryEfficientSafeOpen._get_torch_dtype(dtype_str)
                            break
                
                # Detect scale_input mode
                if "scaled_fp8" in header:
                    metadata = header["scaled_fp8"]
...
class HybridOps(manual_cast):
    ...
    class Linear(manual_cast.Linear):
        ...
        def forward_comfy_cast_weights(self, input):
            ...
            # Access fp8_mat_mult_supported from global config
